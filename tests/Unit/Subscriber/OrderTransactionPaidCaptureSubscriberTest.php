...
class OrderTransactionPaidCaptureSubscriberTest extends TestCase
{
    ...
    public function testCreatesAndCompletesCaptureWhenMultiSafepayTransactionIsPaid(): void
    {
        ...
        $stateMachineStateRepository->expects($this->once())
            ...
            ->with(
                ...
                $this->callback(static fn (Context $writeContext): bool => $writeContext->getVersionId() === 'transaction-version-id')
            )
            ->willReturn(new EntitySearchResult(
                'state_machine_state',
                1,
        ...
        $captureStateHandler->expects($this->once())
            ->method('complete')
            ->with(
                $this->callback(static fn (string $captureId): bool => $captureId !== ''),
                $this->callback(static fn (Context $writeContext): bool => $writeContext->getVersionId() === 'transaction-version-id')
            );
    ...
    public function testCreatesCaptureUsingContextVersionWhenTransactionVersionIsMissing(): void
    {
        ...
        $stateMachineStateRepository = $this->createMock(EntityRepository::class);
        $captureStateHandler = $this->createMock(OrderTransactionCaptureStateHandler::class);
        $transaction = $this->createTransaction($orderTransactionId, null);
        ...
        $orderTransactionCaptureRepository->expects($this->once())
            ->method('create')
            ->with($this->callback(function (array $payload) use ($orderTransactionId, $context): bool {

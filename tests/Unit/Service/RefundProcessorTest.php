...
class RefundProcessorTest extends TestCase
{
    public function testResolveReturnRefundPersistenceKeySkipsExistingCompletedOperation(): void
    {
        ...
        $processor = $this->createProcessor(
            ...
            $this->createRefundRepositoryForExistingKey(true)
        );
    ...
    public function testResolveReturnRefundPersistenceKeyAllowsNewDeltaForSameReturn(): void
    {
        ...
        $processor = $this->createProcessor(
            $this->createCompletedCaptureRepository(),
            $this->createRefundRepositoryForExistingKey(false)
        );
    ...
    public function testGetRefundedAmountCentsFromShopwareReturnIntegrationCountsOnlyCompletedRefunds(): void
    {
        ...
        $processor = $this->createProcessor(
            $this->createCompletedCaptureRepository(),
            $this->createRefundRepositoryForCompletedReturnIntegrationTotal()
        );
    ...
    public function testGetRefundedAmountCentsFromShopwareReturnIntegrationIgnoresMixedCaptureVersionPairs(): void
    {
        ...
        $processor = $this->createProcessor(
            ...
            $this->createRefundRepositoryForMixedCaptureVersionPairs()
        );
    ...
    public function testSyncTransactionRefundStateFromTotalsTransitionsFullRefund(): void
    {
        ...
        $stateHandler->expects($this->once())->method('refund')->with('transaction-id', $context);
        $stateHandler->expects($this->never())->method('refundPartially');
    ...
    public function testSyncTransactionRefundStateFromTotalsTransitionsPartialRefund(): void
    {
        ...
        $stateHandler = $this->createMock(OrderTransactionStateHandler::class);
        $stateHandler->expects($this->once())->method('refundPartially')->with('transaction-id', $context);
        $stateHandler->expects($this->never())->method('refund');
    ...
    private function createProcessor(
        ...
        $transaction->setOrderVersionId(Defaults::LIVE_VERSION);
        ...
        $repository->method('search')
            ->willReturnCallback(static function (Criteria $criteria, Context $context): EntitySearchResult {
                ...
                return new EntitySearchResult(
                    'order_transaction_capture_refund',
                    1,
                    new EntityCollection([self::createCompletedRefundWithDefaultCapture()]),
                    null,
                    $criteria,
                    $context
                );
    ...
    {
        return new class extends OrderTransactionCaptureRefundEntity {
            private StateMachineStateEntity $state;

            public function __construct()
            {
                $this->setUniqueIdentifier('refund-id');
                $this->state = new StateMachineStateEntity();
                $this->state->setTechnicalName(OrderTransactionCaptureRefundStates::STATE_COMPLETED);
            }
